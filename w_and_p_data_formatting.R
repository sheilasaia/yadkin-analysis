...
# ---- 2. reformat WPCOMP data for yadkin counties ----
...
yadkin_counties_sel_simp=arrange(yadkin_counties_sel,COUNTY) %>% select(-COUNTY)

# select only counties in yadkin ws
comp_yadkin_data=left_join(yadkin_counties_sel_simp,comp_nc_county_data,by="NAME")
...
write_csv(comp_yadkin_data,"comp_yadkin_data.csv")


# ---- 3. reformatting WPGEO data for yadkin counties ----
...
for (i in 1:length(yadkin_counties_sel$NAME))
  {
    if (i==1) {
      ...
      geo_yadkin_data=my_table_final
    }
      ...
      my_table_final=my_table_sel %>% 
        ...
        select(var_name,NAME,COUNTY,yr_2000:yr_2009)
      assign(yadkin_counties_sel$NAME[i],my_table_final)
      geo_yadkin_data=bind_rows(geo_yadkin_data,eval(as.name(yadkin_counties_sel$NAME[i])))
    }
  }
...
# export data
write_csv(geo_yadkin_data,"geo_yadkin_data.csv")

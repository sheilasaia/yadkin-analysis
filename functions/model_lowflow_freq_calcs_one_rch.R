...
model_lowflow_freq_calcs_one_rch=function(obs_lowflow_freq_calcs_one_rch_df,kn_table,model_p_list,general_cskew) {
  ...
  current_sub=unique(obs_lowflow_freq_calcs_df$RCH)
  num_p=length(model_p_list)
  num_yrs=dim(obs_lowflow_freq_calcs_df)[1]
  obs_rank_num=obs_lowflow_freq_calcs_df$obs_rank_num
  obs_return_period=obs_lowflow_freq_calcs_df$obs_return_period_yr
  obs_flow_unlog=obs_lowflow_freq_calcs_df$obs_min_flow_cms_adj
  ...
  if (perc_zeros_check<=0.25) {
    if (perc_zeros_check==0) {
      ...
      model_df=data.frame(RCH=rep(current_sub,num_p),
      ...
      if (obs_overzero_cskew<-0.4) {
        obs_flow_low_limit=obs_overzero_mean-kn_value*obs_overzero_stdev

...
# define function
risk_vuln_reclass=function(hiflow_freq_sovi_data) {
  
  # run for each dataset
  # case 1: if high flow freq is above 0 & sovi rescaled is equal to/below 0 (high risk/low vulnerability)
  # case 2: if high flow freq is equal to/below 0 & sovi rescaled is equal to/below 0 (low risk/low vulnerability)
  # case 3: if high flow freq is above 0 & sovi rescaled is above 0 (high risk/high vulnerability)
  # case 4: if high flow freq is equal to/below 0 & sovi rescaled is above 0 (low risk/high vulnerability)
  ...
  num_datasets=length(unique(hiflow_freq_sovi_data$dataset))
  
  # iterate through data
  for (i in 1:num_datasets) {
    
    # select one dataset for analysis
    temp_dataset_name=as.character(unique(hiflow_freq_sovi_data$dataset)[i])
    temp_df=hiflow_freq_sovi_data %>% filter(dataset==temp_dataset_name)

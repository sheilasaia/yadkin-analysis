...
# ---- 3.3 plot low flow freq. results for each subbasin (backcast) ----
...
# ---- 4.1 calculate % change MAGNITUDE OF FLOWS for a return period (backcast) ----
...
# ---- 5.1 calculate % change in NUMBER OF FLOWS at/below a given return period (backcast) ----

# 10 yr
miroc8_5_10yr_n_flow_change = rp_n_flow_change(10, miroc_baseline_model_lowflow_calcs, miroc_baseline_rch_data, miroc8_5_rch_data, flow_option = "lowflow") %>%
...
#  mutate(dataset = "miroc8_5")
#csiro4_5_25yr_n_flow_change = rp_n_flow_change(25, csiro_baseline_model_lowflow_calcs, csiro_baseline_rch_data, csiro4_5_rch_data, flow_option = "lowflow") %>%
#  mutate(dataset = "csiro4_5")
#csiro8_5_25yr_n_flow_change = rp_n_flow_change(25, csiro_baseline_model_lowflow_calcs, csiro_baseline_rch_data, csiro8_5_rch_data, flow_option = "lowflow") %>%
#  mutate(dataset = "csiro8_5")
#hadley4_5_25yr_n_flow_change = rp_n_flow_change(25, hadley_baseline_model_lowflow_calcs, hadley_baseline_rch_data, hadley4_5_rch_data, flow_option = "lowflow") %>%
#  mutate(dataset = "hadley4_5")
...
setwd("/Users/ssaia/Desktop")
cairo_pdf("change_num_lowflows_10yr_map.pdf",width=11,height=8.5)
ggplot(yadkin_subs_shp_n_flow_change_10yr,aes(fill=perc_change_per_yr)) +
  ...
  scale_fill_gradient2("% change in number of days/yr with flows <= 10-yr flow",na.value="grey75", limits = c(-10, 30), high="darkblue",low="darkred") +
  theme_bw()
dev.off()
...
# export results
#setwd("/Users/ssaia/Documents/sociohydro_project/analysis/results/r_outputs")
#write_csv(n_flow_change_10yr_area,"num_lowflow_change_10yr_calcs.csv")
#write_csv(n_flow_change_25yr_area,"num_lowflow_change_25yr_calcs.csv")


# ---- 6.1 count number of days with no flow ----
...
baseline_obs_no_flow_counts = baseline_rch_data %>%
  ...
  mutate(n_no_flow_per_yr = sum_n_no_flow_entries / baseline_num_yrs,
...
miroc_baseline_obs_no_flow_counts = miroc_baseline_rch_data %>%
  group_by(RCH) %>% 
  summarise(sum_n_no_flow_entries = sum(FLOW_OUTcms == 0)) %>%
  mutate(n_no_flow_per_yr = sum_n_no_flow_entries / miroc_baseline_num_yrs,
...
miroc8_5_obs_no_flow_counts = miroc8_5_rch_data %>% 
  group_by(RCH) %>% 
  summarize(sum_n_no_flow_entries = sum(FLOW_OUTcms == 0)) %>%
  mutate(n_no_flow_per_yr = sum_n_no_flow_entries / miroc8_5_num_yrs,
...
csiro_baseline_obs_no_flow_counts = csiro_baseline_rch_data %>%
  group_by(RCH) %>% 
  summarise(sum_n_no_flow_entries = sum(FLOW_OUTcms == 0)) %>%
  mutate(n_no_flow_per_yr = sum_n_no_flow_entries / csiro_baseline_num_yrs,
...
csiro8_5_obs_no_flow_counts = csiro8_5_rch_data %>%
  group_by(RCH) %>% 
  summarise(sum_n_no_flow_entries = sum(FLOW_OUTcms == 0)) %>%
  mutate(n_no_flow_per_yr = sum_n_no_flow_entries / csiro8_5_num_yrs,
...
csiro4_5_obs_no_flow_counts = csiro4_5_rch_data %>%
  group_by(RCH) %>% 
  summarise(sum_n_no_flow_entries = sum(FLOW_OUTcms == 0)) %>%
  mutate(n_no_flow_per_yr = sum_n_no_flow_entries / csiro4_5_num_yrs,
...
hadley_baseline_obs_no_flow_counts = hadley_baseline_rch_data %>%
  group_by(RCH) %>% 
  summarise(sum_n_no_flow_entries = sum(FLOW_OUTcms == 0)) %>%
  mutate(n_no_flow_per_yr = sum_n_no_flow_entries / hadley_baseline_num_yrs,
...
hadley4_5_obs_no_flow_counts = hadley4_5_rch_data %>%
  group_by(RCH) %>% 
  summarise(sum_n_no_flow_entries = sum(FLOW_OUTcms == 0)) %>%
  mutate(n_no_flow_per_yr = sum_n_no_flow_entries / hadley4_5_num_yrs,
...
# ---- 6.2 calculate % change in NUMBER OF DAYS with no flow (backcast) ----
...
# calculate % change 
miroc8_5_no_flow_change_using_baseline = no_flow_change(miroc_baseline_obs_no_flow_counts, miroc8_5_obs_no_flow_counts, baseline_num_yrs)
csiro8_5_no_flow_change_using_baseline = no_flow_change(csiro_baseline_obs_no_flow_counts, csiro8_5_obs_no_flow_counts, baseline_num_yrs)
csiro4_5_no_flow_change_using_baseline = no_flow_change(csiro_baseline_obs_no_flow_counts, csiro4_5_obs_no_flow_counts, baseline_num_yrs)
hadley4_5_no_flow_change_using_baseline = no_flow_change(hadley_baseline_obs_no_flow_counts, hadley4_5_obs_no_flow_counts, baseline_num_yrs)

# bind rows
n_no_flow_change=bind_rows(miroc8_5_no_flow_change_using_baseline,
                           csiro8_5_no_flow_change_using_baseline,
                           csiro4_5_no_flow_change_using_baseline,
                           hadley4_5_no_flow_change_using_baseline) %>%
  mutate(SUB=RCH)

# add to shp file
yadkin_subs_shp_no_flow_using_baseline = left_join(yadkin_subs_shp, n_no_flow_change, by = "SUB")
#glimpse(yadkin_subs_shp_no_flow_using_baseline)

# adjust levels
n_no_flow_change$dataset = factor(n_no_flow_change$dataset, levels = c("miroc8_5", "csiro8_5", "csiro4_5", "hadley4_5"))
yadkin_subs_shp_no_flow_using_baseline$dataset = factor(yadkin_subs_shp_no_flow_using_baseline$dataset, levels = c("miroc8_5", "csiro8_5", "csiro4_5", "hadley4_5"))


# ---- 6.3 plot bar chart per year facetted by subbasin (no backcast and backcast) ----

#setwd("/Users/ssaia/Desktop")
#cairo_pdf("no_flow_counts_per_yr_by_sub.pdf",width=11,height=8.5)
...
ggplot(all_models_no_flow_counts_by_sub,aes(x=dataset,y=n_no_flow_per_yr,fill=datatype)) +
  geom_col() +
  facet_wrap(~RCH,ncol=7,nrow=4) +
  xlab("") +
...
# ---- 6.4 plot bar chart of total number days with no flow per year by dataset (sum of subbasins, no backcast and backcast) ----
...
all_models_no_flow_counts = all_models_no_flow_counts_by_sub %>% 
  ...
  summarize(sum_n_no_flow_entries_by_dataset = sum(sum_n_no_flow_entries),
            sum_n_no_flow_entries_by_dataset_per_yr_per_subbasin = sum(sum_n_no_flow_entries) / baseline_num_yrs / num_subs)

#setwd("/Users/ssaia/Desktop")
#cairo_pdf("no_flow_counts_per_yr_by_dataset.pdf",width=11,height=8.5)
...
ggplot(all_models_no_flow_counts,aes(x=dataset,y=sum_n_no_flow_entries_by_dataset_per_yr_per_subbasin,fill=datatype)) +
  geom_col() +
  xlab("") +
...
# ---- 6.5 plot map (backcast) ----

# results below 50 % change
setwd("/Users/ssaia/Desktop")
cairo_pdf("change_num_no_flow_map_lessthan50perc.pdf", width = 11, height = 8.5)
ggplot(yadkin_subs_shp_no_flow_using_baseline, aes(fill = perc_change_per_yr)) +
  facet_wrap(~dataset) +
  geom_sf() +
...
# results above 50 % change
setwd("/Users/ssaia/Desktop")
cairo_pdf("change_num_no_flow_map_over50perc.pdf", width = 11, height = 8.5)
ggplot(yadkin_subs_shp_no_flow_using_baseline, aes(fill = perc_change_per_yr)) +
  facet_wrap(~dataset) +
  geom_sf() +
...
# ---- 6.6 calcualte variation (backcast) ----
...
# join areas
n_no_flow_change_area = n_no_flow_change %>%
  left_join(contributing_areas, by = 'SUB')

# backcast baseline (and recode them for plotting)
n_no_flow_change_baseline = n_no_flow_change_area %>%
  select(SUB, AREAkm2, n_base_flows, dataset) %>%
  mutate(n_base_flows_per_yr = n_base_flows / baseline_num_yrs) %>%
  filter(dataset!= "csiro4_5") # don't need both CSIRO datasets b/c backcast baselines are the same for both
n_no_flow_change_baseline$dataset = recode(n_no_flow_change_baseline$dataset, "miroc8_5" = "MIROC", "csiro8_5" = "CSIRO", "hadley4_5" = "Hadley")

# backcast baseline ordered by subbasin area
n_no_flow_change_baseline$SUB = factor(n_no_flow_change_baseline$SUB, levels = contributing_areas$SUB[order(contributing_areas$AREAkm2)])
n_no_flow_change_baseline$dataset = factor(n_no_flow_change_baseline$dataset,levels = c("MIROC", "CSIRO", "Hadley"))

# backcast baseline summary for pointrange plot
n_no_flow_change_baseline_summary = n_no_flow_change_baseline %>%
  group_by(SUB, AREAkm2) %>%
  summarize(min_n_no_flow_per_yr = min(n_base_flows_per_yr),
            max_n_no_flow_per_yr = max(n_base_flows_per_yr),
            mean_n_no_flow_per_yr = mean(n_base_flows_per_yr))


# projections
n_no_flow_change_projection = n_no_flow_change_area %>%
  select(SUB, AREAkm2, n_proj_flows, dataset) %>%
  mutate(n_proj_flows_per_yr = n_proj_flows / miroc8_5_num_yrs) # all are equal to 21 but use miroc8_5_num_yrs for simplicity 

# projections ordered by subbasin area
n_no_flow_change_projection$SUB = factor(n_no_flow_change_projection$SUB, levels = contributing_areas$SUB[order(contributing_areas$AREAkm2)])
n_no_flow_change_projection$dataset = factor(n_no_flow_change_projection$dataset, levels = c("miroc8_5", "csiro8_5", "csiro4_5", "hadley4_5"))

# projections summary for pointrange plot
n_no_flow_change_projection_summary = n_no_flow_change_projection %>%
  group_by(SUB, AREAkm2) %>%
  summarize(min_n_no_flow_per_yr = min(n_proj_flows_per_yr),
            max_n_no_flow_per_yr = max(n_proj_flows_per_yr),
            mean_n_no_flow_per_yr = mean(n_proj_flows_per_yr))


# ---- 6.7 plot variation (backcast) ----

# make a list to hold plots
my_no_flow_plots = list()

# backcast baseline variation plot
my_no_flow_plots[[1]] = ggplot() +
  geom_pointrange(data=n_no_flow_change_baseline_summary,
                  aes(x=SUB,y=mean_n_no_flow_per_yr,ymin=min_n_no_flow_per_yr,ymax=max_n_no_flow_per_yr),shape=32) +
  geom_point(data=n_no_flow_change_baseline,aes(x=SUB,y=n_base_flows_per_yr,color=dataset),
...
my_no_flow_plots[[2]] = ggplot() +
  geom_pointrange(data=n_no_flow_change_projection_summary,
                  aes(x=SUB,y=mean_n_no_flow_per_yr,ymin=min_n_no_flow_per_yr,ymax=max_n_no_flow_per_yr),
                  shape=32) +
  geom_point(data=n_no_flow_change_projection,aes(x=SUB,y=n_proj_flows_per_yr,color=dataset),
...
# save plot
setwd("/Users/ssaia/Desktop")
cairo_pdf("num_no_flow_variation.pdf",width = 15, height = 8.5, pointsize = 18)
multiplot(plotlist = my_no_flow_plots, cols = 2)
dev.off()
...
n_no_flow_change_sel=n_no_flow_change %>%
  select(SUB,dataset,no_flow_perc_change,no_flow_perc_change_per_yr)

# export to results
#setwd("/Users/ssaia/Documents/sociohydro_project/analysis/results/r_outputs")
#write_csv(n_no_flow_change_sel,"no_flow_perc_change_data.csv")
#write_csv(contributing_areas,"subbasin_contributing_areas_data.csv")


# ---- 8.1 check normality of data (backcast) ----
...
# ---- 8.2 calculate outlier cutoffs and number of outlier low flows (backcast) ----
...
# ---- 8.3 calculate % change in NUMBER OF MINOR OUTLIER low flows (backcast) ----
...
# calculate % change 
miroc8_5_lowflow_outlier_change_using_baseline=outlier_change(miroc_baseline_outlier_counts_sum,miroc8_5_outlier_counts_using_baseline_sum,flow_option="lowflow",baseline_num_yrs)
csiro8_5_lowflow_outlier_change_using_baseline=outlier_change(csiro_baseline_outlier_counts_sum,csiro8_5_outlier_counts_using_baseline_sum,flow_option="lowflow",baseline_num_yrs)
csiro4_5_lowflow_outlier_change_using_baseline=outlier_change(csiro_baseline_outlier_counts_sum,csiro4_5_outlier_counts_using_baseline_sum,flow_option="lowflow",baseline_num_yrs)
hadley4_5_lowflow_outlier_change_using_baseline=outlier_change(hadley_baseline_outlier_counts_sum,hadley4_5_outlier_counts_using_baseline_sum,flow_option="lowflow",baseline_num_yrs)

# bind rows
all_models_lowflow_outlier_change=bind_rows(miroc8_5_lowflow_outlier_change_using_baseline,
                                            csiro8_5_lowflow_outlier_change_using_baseline,
                                            csiro4_5_lowflow_outlier_change_using_baseline,
                                            hadley4_5_lowflow_outlier_change_using_baseline) %>% 
  mutate(SUB=RCH) %>% 
  select(-RCH)

# add to shp file
yadkin_subs_shp_lowflow_outliers_using_baseline=left_join(yadkin_subs_shp,all_models_lowflow_outlier_change,by="SUB")
#glimpse(yadkin_subs_shp_lowflow_outliers_using_baseline)

# adjust levels
yadkin_subs_shp_lowflow_outliers_using_baseline$dataset=factor(yadkin_subs_shp_lowflow_outliers_using_baseline$dataset,levels=c("miroc8_5","csiro8_5","csiro4_5","hadley4_5"))


# ---- 8.4 plot variation (backcast) ----
...
ggplot() +
  ...
  geom_point(data=lowflow_change_baseline,aes(x=SUB,y=baseline_sum_n_minor_outliers_per_yr,color=dataset),
             shape=17,size=5,alpha=0.75, position=position_jitter(height=0.1,width=0)) +
  #geom_smooth(method='loess',formula=y~x) +
...
ggplot() +
  geom_pointrange(data=lowflow_change_projection_summary,
                  ...
                  shape=32) +
  geom_point(data=lowflow_change_projection,aes(x=SUB,y=projection_sum_n_minor_outliers_per_yr,color=dataset),
             size=5,alpha=0.75, position=position_jitter(height=0.1,width=0)) +
  #geom_smooth(method='loess',formula=y~x) +
  xlab("SWAT Subbasin Number (by Increasing Conbributing Area)") +
  ylab("Number of Minor LOFs/yr") +
...
# ---- 8.5 plot boxplots for low flow outlier context (backcast) ----

ggplot(all_models_lowflow_outlier_change) +
  geom_point(aes(x=dataset,y=minor_outlier_perc_change))

# ---- 8.6 plot map (backcast) ----

# minor outliers (up to 100% change)
setwd("/Users/ssaia/Desktop")
cairo_pdf("lowflow_minor_outlier_change_using_baseline_low.pdf",width=11,height=8.5)
ggplot(yadkin_subs_shp_lowflow_outliers_using_baseline,aes(fill=minor_outlier_perc_change_per_yr)) +
  facet_wrap(~dataset) +
  geom_sf() +
  coord_sf(crs=st_crs(102003)) + # yadkin_subs_shp_lowflow_outliers_using_baseline is base utm 17N so convert to Albers for CONUS
...
setwd("/Users/ssaia/Desktop")
cairo_pdf("lowflow_minor_outlier_change_using_baseline_up.pdf",width=11,height=8.5)
ggplot(yadkin_subs_shp_lowflow_outliers_using_baseline,aes(fill=minor_outlier_perc_change_per_yr)) +
  facet_wrap(~dataset) +
  geom_sf() +
  coord_sf(crs=st_crs(102003)) + # yadkin_subs_shp_lowflow_outliers_using_baseline is base utm 17N so convert to Albers for CONUS
...
ggplot(yadkin_subs_shp_lowflow_outliers_using_baseline,aes(fill=major_outlier_perc_change)) +
  facet_wrap(~dataset) +
  geom_sf() +
  coord_sf(crs=st_crs(102003)) + # yadkin_subs_shp_lowflow_outliers_using_baseline is base utm 17N so convert to Albers for CONUS
...
# ---- 3.x plot low flow freq. results for each subbasin (no backcast) ----
...
# ---- 4.x calculate % change MAGNITUDE OF FLOWS for given return period (no backcast) ----
...
# ---- 4.x export results (no backcast) ----
